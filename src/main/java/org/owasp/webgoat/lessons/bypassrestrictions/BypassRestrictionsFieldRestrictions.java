...
public class BypassRestrictionsFieldRestrictions implements AssignmentEndpoint {
  ...
  // These restrictions (allowed select/radio values, checkbox on/off, a 5 character max
  // length, a readonly value that must not change) were only ever enforced in the HTML;
  // the server accepted whatever the client sent. Enforce the same restrictions here so a
  // request that violates them server-side is rejected, not rewarded.
